...
MATRIX_SHARED_SECRET = os.environ.get("MATRIX_SHARED_SECRET", "C&1gRZ#;M2hEp-ehNLtSPeddl^DOutp*Ls4=eDyx_+._^Y#ieY")
...
async def run_matrix_e2e_negotiation(
    ...
    async with httpx.AsyncClient(timeout=30.0) as http:
        ...
        tick_seen = False
        for _ in range(60):
            r = await http.get(f"{BACKEND_URL}/rooms/{quote(session_room, safe='')}/messages", params={"limit": 30})
            if r.status_code == 200:
...
async def test_matrix_two_agent_negotiation(test_ctx: TestContext):
    ...
    agents_config = [
        ("matrix-features", "Features Team", "Prioritize new features. We need 70% capacity for roadmap items."),
        ("matrix-stability", "Stability Team", "Focus on stability. Need 60% capacity for bug fixes and tech debt."),
    ]
    ...
    try:
        ...
        success, consensus = await run_matrix_e2e_negotiation(ctx, agents_config, "Sprint Capacity Allocation")
        ...
        check(
            test_ctx,
            "coordination_consensus received",
            success and consensus is not None,
...
async def test_matrix_three_agent_negotiation(test_ctx: TestContext):
    ...
    agents_config = [
        ("matrix-speed", "Speed Advocate", "Release ASAP with minimal testing. Speed to market is critical."),
        ("matrix-quality", "Quality Advocate", "Comprehensive testing required. Quality issues damage reputation."),
        ("matrix-cost", "Cost Advocate", "Minimize resources. Staged rollout balances speed and quality."),
    ]
    ...
    try:
        ...
        success, consensus = await run_matrix_e2e_negotiation(ctx, agents_config, "Software Release Planning")
...
async def test_matrix_architecture_decision(test_ctx: TestContext):
    ...
    agents_config = [
        ("matrix-postgres", "PostgreSQL Advocate", "PostgreSQL: ACID compliance, mature ecosystem, pgvector for AI."),
        ("matrix-mongo", "MongoDB Advocate", "MongoDB: schema flexibility, horizontal scaling, faster iteration."),
    ]
    ...
    try:
        ...
        success, consensus = await run_matrix_e2e_negotiation(ctx, agents_config, "Database Technology Selection")
...
async def main():
    ...
    print("Agents coordinate through Matrix + Mycelium IOC.\n")
